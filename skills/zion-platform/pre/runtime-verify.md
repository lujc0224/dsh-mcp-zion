...
```bash
npx -y zion-mcp@2.6.2 runtime graphql --args '{"query":"query { <root_op> { ... } }","variables":{}}'
...
```bash
npx -y zion-mcp@2.6.2 runtime query --tableName order --where '{"status":{"_eq":"PAID"}}' --fields '["id","status","total"]'
npx -y zion-mcp@2.6.2 runtime insert --tableName order --objects '[{"status":"PAID","total":100}]'
npx -y zion-mcp@2.6.2 runtime update --tableName order --where '{"id":{"_eq":42}}' --set '{"status":"SHIPPED"}'
npx -y zion-mcp@2.6.2 runtime delete --tableName order --where '{"id":{"_eq":42}}'
npx -y zion-mcp@2.6.2 runtime graphql --query 'query { order_aggregate { aggregate { count } } }'
npx -y zion-mcp@2.6.2 runtime run-code --jsCode 'const total = 2 + 2; total;'
```
...
### Yours alone
Both you and the in-editor agent can deploy the backend — `npx -y zion-mcp@2.6.2 project sync-backend` here,
...
- `npx -y zion-mcp@2.6.2 schema validate` — type-check the loaded schema before deploying it.
- `npx -y zion-mcp@2.6.2 site deploy` — ship a built frontend directory, with `site status` / `site abort` for the run; a PROD target needs a human to approve it.
