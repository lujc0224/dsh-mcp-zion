...
### Data Sources Available in Zion
...
A number, boolean, date, time, datetime or color bound into a `TEXT` slot is converted for display automatically, so bind the option straight to the slot — never wrap it in `toText` or any other conversion operator. Only structured values (lists, objects, media) genuinely need one. `BROWSE_DATA_BINDING_OPTIONS` marks an option with `*` only when the target really cannot take it; an unmarked option needs no conversion step.
